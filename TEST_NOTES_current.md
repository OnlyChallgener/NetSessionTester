# v1.0.21-beta3 build152 自测重点

## 版本与发版文件

1. APP 版本应显示 `versionName=v1.0.21-beta3`、`versionCode=152`。
...
### 1. 7 天历史记录体系（缓冲评级、双网对测、iPerf3）
- 分别运行一次“缓冲评级”、“双网对测”和“iPerf3测速”，验证测试完毕后历史记录是否自动持久化。
- 检查历史记录卡片顶部是否显示记录条数与占用大小（如“已保存 1 条（保留7天）· 占用 0.8 KB”）。
- 检查每天的记录是否自动折叠，点击日期标题栏可展开/收起。
- 检查单条记录向左滑动是否露出红色垃圾桶按钮，点击是否成功删除该条记录并实时刷新占用 KB。

### 2. iPerf3 页面排版验证
- 进入 iPerf3 页面，检查“服务器”与“端口”行是否排版整洁自然，端口输入框不再被卡片右侧遮挡截断，并验证常用端口快捷胶囊（5201/5202/5203）。

### 3. NAT 与 MTU 优化验证
- 运行 NAT 诊断，验证国内高可用 STUN 池响应速度，检查是否准确识别 Double NAT/CGNAT 及局域网 UPnP 自动打洞状态。
- 运行 MTU 检测，验证在常规 1500 局域网下是否在 1~2 秒内极速出结果，并查看主机/路由器场景化调优建议。
