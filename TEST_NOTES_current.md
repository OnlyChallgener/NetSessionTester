...
1. APP 版本应显示 `versionName=v1.0.21`、`versionCode=156`。
...
### 1. 超高 CPS 发射与突破 2000 限制测试
- 在设置或测试主页输入目标 CPS：例如 `3000`、`5000`；
- 在高速局域网或高性能测试目标下启动测试，观察“目标CPS”与“实际CPS”是否成功突破 2000 上限并达到对应速率；
- 检查启动初期是否平滑发射，无早期被在飞失败保护拦截而引发的降速；
- 观察并发连接数增长折线是否平稳上升。

### 2. 底栏 4-Tab 架构与独立工具中心
- 检查底部导航栏是否呈现 4 个 Tab（测试、工具、历史、设置），默认选中「测试」，间距与图标对称协调。
- 进入「工具」Tab，检查网络诊断、性能测试、运营商工参分类卡片流，每个工具均可点击进入或唤起弹窗。
- 检查 WiFi 漫游已归入“性能测试”（AP无缝漫游 · 切换丢包与时延），运营商工参已与 WiFi 解耦；原误导性双网对测已移除。

### 3. 连接测试与折线图完整性验证（解决假死卡住与无折线）
- 分别在 IPv4、仅 IPv6、IPv4+IPv6 模式下启动连接数测试：
  - 在无 IPv6 或地址解析失败时，检查是否立即汇报“解析失败”而非一直卡在“运行中/待测试”；
  - 检查折线图是否能正常显示折线点，未出现异常时显示“开始测试后显示本次折线”；
  - 测试过程中或快速失败后点击“停止”，检查是否能立即可靠退出并恢复“开始”按钮。

### 4. 关闭自动 NAT 与强化手动诊断
- 检查进入 App 或刷新网络时，主页不再后台静默发起 STUN 探测；
- 点击 NAT 诊断，检查弹窗中卡片适度加宽，测试结果与进度置顶呈现免向下滚动，恢复单一默认 RFC5780 权威服务器。

### 5. 缓冲评级、iPerf3 目标弹出选择器
- 分别进入两大页面，点击目标/服务器输入框旁的弹出选择器；
- 检查是否显示 5 个推荐目标（iPerf3 包含内网网关 192.168.5.1）以及自定义目标列表；
- 尝试添加自定义目标（最多 5 个），测试点击填入与滑动/点击垃圾桶删除。

## 历史新增功能回归重点

### 1. 内置 iPerf3 客户端
- 从 工具 -> 性能测试 -> 点击“iPerf3测速”。
- 输入内网服务器（如软路由/网关 `192.168.5.1`）或公网 iPerf3 节点，端口默认 5201。
