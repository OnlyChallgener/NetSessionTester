...
## V1.1.15 build128 - 高频 Ping TCP Socket 自测

- Ping 高频模式增加 TCP Socket 引擎。
- 间隔 < 200ms 时优先探测 TCP 端口并用 Socket connect 延迟作为高频 RTT。
- 修复无限高频 Ping 仍走串行 ICMP 导致实际频率只有 5-8 次/s 的问题。
- 系统 ping 流式探测支持无限模式分块执行。
- ICMP 流式 ping 间隔下限改为 25ms。
- 无可用 TCP 端口时回退 ICMP，并记录高频受限提示。
...
## V1.1.15 build127 selftest
- Ping 历史最大保留条数从 8 调整为 12。
- Ping 历史二级页标题显示当前条数/上限和占用空间。
- Ping 历史单条卡片增加删除操作。
- 连接数测试启动时，Ping 监测自动使用连接数测试同一个目标地址，并重置旧曲线。
- Ping 图表增加低延迟稳定场景的局部 Y 轴放大，改善路由器 Ping 曲线长期挤在上半部分的问题。
