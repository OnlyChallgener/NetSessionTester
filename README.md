...
## v0.9.8 Diagnosis / Axis Refinement

- Ping 卡片去掉“已运行”小框，运行时间通过 X 轴体现。
- 诊断建议增强：仅 IPv4 / 仅 IPv6 / FD上限 / 异常中断 / 失败连接都会给出实际建议，不再只显示泛化文案。
- 会话图表 Y 轴规则调整：低于 2500 时峰值显示 2500，不再使用 2000 档。
- 版本号保持 v0.9.8 不变。
