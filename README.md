...
## v0.9.9 build 62 - FD/Socket 上限保护

- 接近 Android FD/Socket 上限前提前停止新增连接，预留系统资源用于 UI、日志、历史保存和释放。
- 增加堆内存压力保护，避免极限连接数附近出现 OOM 闪退。
- openBatch / openOne 增加兜底保护，FD、资源压力、异常均转换为测试结果并触发统一收尾。
- 后台 close 异常不再影响“已释放”状态和历史保存。

## v0.9.9 build 62 - 网络诊断表达优化
